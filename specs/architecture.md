...
- [ ] How should the `--registry` flag interact with fully qualified image references like `registry.example.com/repo:tag`? (Recommendation: parse the reference and use embedded registry if `--registry` not provided.)
